...
class App():
    def __init__(self):
        ...
        self.cube = Cube.Cube(position=(190, 180), size=1, fov=256, viewer_distance=4)
    ...
    def main(self):
        ...
        while self.running:
            ...
            if self.fade_in_alpha > 0:
                self.fade_in_alpha = max(0, self.fade_in_alpha - 60)  # Vitesse augmentée nettement
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if result_emergency == "emergency_stop":
                        ...
                        if not start_screen_module.USE_PHONE_SENSORS and self.socket_client is not None and hasattr(self.socket_client, 'running') and self.socket_client.running:
                ...
                with self.data_handler.data_lock:
                    ...
                    try:
                        if "AccX" in data_text.keys():
                            self.roll = data_text["AngleRoll"] + 90
                            self.pitch = data_text["AnglePitch"] + 90
                            self.yaw = data_text["AnglaYaw"]
                    except:
                        pass
            ...
            roll_value_text = self.font15.render(f"ROLL: {int(self.roll)}", True, (255,0,0))
            pitch_value_text = self.font15.render(f"PITCH: {int(self.pitch)} ", True, (0,255,0))
            yaw_value_text = self.font15.render(f"YAW: {int(self.yaw)} ", True, BLUE)
            ...
            speed_text_lm = self.font.render(self.input_text_lm if self.input_active_lm else str(self.speed_clock_lm.speed), True, (255, 255, 255))
            text_rect_lm = speed_text_lm.get_rect(center=(535,710))
            self.virtual_screen.blit(speed_text_lm, text_rect_lm)
            speed_text_rm = self.font.render(self.input_text_rm if self.input_active_rm else str(self.speed_clock_rm.speed), True, (255, 255, 255))
            ...
            if globals().get('WINDOW_BG'):
                ...
                if cached_data.get('surface'):
                    self.screen.blit(cached_data['surface'], cached_data['offset'])
                else:
