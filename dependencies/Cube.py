...
def rotate_point(point, angle_x, angle_y, angle_z):
    ...
    # Rotation autour de Y (YAW) en premier - rotation horizontale
    ...
    # Rotation autour de X (PITCH) en dernier
...
class Cube(pygame.sprite.Sprite):
    def __init__(self, position=(200, 200), size=4, fov=256, viewer_distance=4):
        ...
        # La face avant (Z+) est la référence - elle reste de face
        self.base_angle_x = 0.0    # Pitch (R/F) - rotation autour de X
        self.base_angle_y = 210.0  # Yaw (A/E) - rotation autour de Y (restauré)
        self.base_angle_z = 0.0    # Roll (Q/D) - rotation autour de Z
        ...
        # Face avant du cube (indices des sommets) - face Z positive
    ...
    def update(self, angle_x, angle_y, angle_z):
        # Combiner les angles de base (contrôlés au clavier) avec les angles des capteurs
        self.angle_x = (self.base_angle_x + angle_x) % 360  
        self.angle_y = (self.base_angle_y + angle_y) % 360  
        self.angle_z = (self.base_angle_z + angle_z) % 360  
        ...
        # Afficher toutes les flèches actives simultanément
        ...
        if abs(dx) > eps or abs(dy) > eps or abs(dz) > eps:
            ...
            if abs(dy) > eps:
                end = z_pos if dy > 0 else z_neg
                self._draw_arrow(self.image, center, end, YELLOW, thickness=2, head_len=10, head_w=6)
            if abs(dz) > eps:
                end = y_pos if dz > 0 else y_neg
                self._draw_arrow(self.image, center, end, YELLOW, thickness=2, head_len=10, head_w=6)
